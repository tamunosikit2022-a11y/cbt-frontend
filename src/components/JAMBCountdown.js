...
function getUrgencyStyle(days) {
  if (days <= 7)  return { bg: "linear-gradient(135deg,#e17055,#d63031)", text: "#FFFFFF", border: "#e1705588" };
  if (days <= 14) return { bg: "linear-gradient(135deg,#fdcb6e,#e17055)", text: "#FFFFFF", border: "#fdcb6e88" };
  if (days <= 30) return { bg: "linear-gradient(135deg,#6c63ff,#a29bfe)", text: "#FFFFFF", border: "#6c63ff88" };
  return { bg: "linear-gradient(135deg,#00b894,#00cec9)", text: "#FFFFFF", border: "#00b89488" };
}
...
export default function JAMBCountdown({ avgScore = 0 }) {
    ...
    return (
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>Set your exam date for a live countdown and daily reminders.</div>
        </div>
        {showPicker ? (
  ...
  if (daysLeft < 0) {
    return (
      <div style={{ background: "var(--surface)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "12px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 22 }}>🎓</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 800, fontSize: 13, color: "#fff" }}>JAMB done! How did it go?</div>
          <div style={{ fontSize: 11, color: "var(--text-muted)" }}>Keep practicing for Post-UTME.</div>
        </div>
        <button onClick={handleClear} style={{ padding: "6px 10px", background: "none", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, color: "var(--text-muted)", fontSize: 11, cursor: "pointer" }}>Clear</button>
      </div>
    );
  }
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
          <button
            ...
            style={{ padding: "7px 10px", background: "var(--border)", backdropFilter: "blur(4px)", color: "#fff", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 9, fontWeight: 800, fontSize: 11, cursor: "pointer" }}>
            Practice →
          </button>
          <button
            onClick={() => setDismissed(true)}
            style={{ padding: "4px 10px", background: "none", border: "none", color: "var(--text-muted)", fontSize: 10, cursor: "pointer" }}>
            hide
          </button>
        </div>
      </div>
