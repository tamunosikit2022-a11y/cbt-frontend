  ...
  const uploadOne = async (file, index, resRef, maxRetries = 2) => {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        const timeoutMs = Math.max(180_000, Math.ceil(file.size / (1024*1024)) * 15_000);
        ...
        const d = await r.json();
        if (r.status === 409) {
          resRef[index] = { name:file.name, size:file.size, status:"duplicate", error: d.error || "Already uploaded" };
          setResults([...resRef]);
          return null;
        }
