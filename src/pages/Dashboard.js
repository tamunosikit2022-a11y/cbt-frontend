...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { icon: "🎯",  label: "Cutoff Tracker",    path: "/cutoffs" },
      ...
      { icon: "🔎",  label: "School Finder",    path: "/school-finder" },
      { icon: "🧾",  label: "Error Review",     path: "/error-review" },
    ],
  },
  {
    label: "Study Tools",
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "🎓",  label: "University Scoreboard", path: "/university-leaderboard" },
      { icon: "🔥",  label: "Challenge Modes", path: "/challenges" },
      { icon: "🏫",  label: "School Competition", path: "/school-competition" },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "🎁",  label: "Daily Rewards", path: "/rewards" },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "💬",  label: "Social / Chat",       path: "/social" },
      { icon: "🌍",  label: "Community Chat",      path: "/community-chat" },
    ],
  },
  {
    label: "Account",
    ...
    <aside style={{
      ...
      <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:14 }}>
        <div style={{ background:`linear-gradient(135deg,rgba(245,158,11,.15),rgba(124,92,255,.12))`, border:`1px solid rgba(245,158,11,.3)`, borderRadius:14, padding:12 }}>
          ...
          <div onClick={() => nav("/subscribe")} style={{
    ...
    <div style={{ padding: mobile ? "16px 14px 80px" : "24px 28px 60px", display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns: mobile ? "1fr" : "1.4fr 1fr", gap:18 }}>
        ...
        <div style={{ background:C.surf, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
          ...
          <div onClick={() => nav("/predicted")} style={{
            display:"flex", alignItems:"center", justifyContent:"space-between",
            marginTop:14, padding:"10px 12px", borderRadius:10,
            background:"rgba(124,92,255,.10)", border:`1px solid ${C.border}`, cursor:"pointer",
          }}>
            <span style={{ fontSize:12.5, fontWeight:700, color:C.text }}>🎓 See your predicted JAMB score</span>
            <span style={{ fontSize:12, color:C.pL, fontWeight:700 }}>View →</span>
          </div>
        </div>
      ...
      {/* Recent exams — FIX (not mobile-first): this used to be a 5-column
          <table> (Subject/Type/Score/Date/Result). Even wrapped in
          overflowX:auto, a 5-column table on a ~360-400px phone screen
          means the content either gets crushed unreadably small or the
          user has to discover and perform a sideways scroll on their most-
          used dashboard widget — a well-known mobile UX anti-pattern.
          Replaced with stacked rows: subject+type on one line, score/date/
          result on the next, everything readable at native size with zero
      ...
      <div style={{ background:C.surf, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
        ...
        {(sessions.length > 0
          ? sessions.slice(0,5).map(s => ({
              sub:  s.subject || "General",
              type: s.exam_type || "JAMB",
              pct:  parseFloat(s.percentage || 0).toFixed(0),
              date: new Date(s.created_at).toLocaleDateString("en-NG", { day:"numeric", month:"short", year:"numeric" }),
              pass: parseInt(parseFloat(s.percentage || 0).toFixed(0)) >= 50,
            }))
          : [
          ...
          <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, padding:"12px 0", borderBottom: i < arr.length-1 ? `1px solid ${C.border}` : "none" }}>
            <div style={{ minWidth:0 }}>
              <div style={{ fontSize:13, fontWeight:700, color:C.text, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{r.sub}</div>
              <div style={{ fontSize:10.5, fontWeight:700, color:C.muted, marginTop:2 }}>{r.type} · {r.date}</div>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
              <span style={{ fontSize:14, fontWeight:800, color:C.text }}>{r.pct}%</span>
              <span style={{ fontSize:10.5, fontWeight:800, padding:"4px 9px", borderRadius:7, background: r.pass ? "rgba(34,197,94,.15)" : "rgba(239,68,68,.15)", color: r.pass ? C.ok : C.danger, whiteSpace:"nowrap" }}>
                {r.pass ? "Passed" : "Below 50%"}
              </span>
            </div>
          </div>
        ))}
