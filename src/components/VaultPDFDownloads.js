...
function EngagementRow({ pdf, onReact }) {
  const [likeCount,    setLikeCount]    = useState(pdf.like_count    || 0);
  const [dislikeCount, setDislikeCount] = useState(pdf.dislike_count || 0);
  const [viewCount]                     = useState(pdf.view_count    || 0);
  const [myReaction,   setMyReaction]   = useState(pdf.my_reaction   || null);
  const [busy, setBusy] = useState(false);

  const react = async (reaction) => {
    if (busy) return;
    setBusy(true);
    // Optimistic update so it feels instant on mobile
    ...
    try {
      const res = await onReact(pdf.id, reaction);
      if (res) {
  ...
  const chip = (active, color) => ({
    ...
    border: `1px solid ${active ? color + "70" : "#374151"}`,
  ...
  return (
    <div style={{ display: "flex", gap: 8, marginTop: 9, flexWrap: "wrap" }}>
      <button onClick={() => react("like")} style={chip(myReaction === "like", "#EC4899")}>
        {myReaction === "like" ? "❤️" : "🤍"} {likeCount}
      </button>
      <button onClick={() => react("dislike")} style={chip(myReaction === "dislike", "#EF4444")}>
        👎 {dislikeCount}
      </button>
      <span style={{ ...chip(false, "#6B7280"), cursor: "default" }}>
        👁 {viewCount}
      </span>
    </div>
  );
...
function PDFCard({ pdf, onDownload, downloading, onReact }) {
  ...
  return (
    ...
    >
      ...
      {/* Likes / dislikes / views — same idea as the ProjectPQ-style cards */}
      <EngagementRow pdf={pdf} onReact={onReact} />
...
export default function VaultPDFDownloads() {
  ...
  const fetchPdfs = useCallback(async () => {
    ...
    try {
      ...
      // FIX: this was calling /vault/pdfs, but the backend actually mounts
      // these student PDF routes at /api/pdfs (see server.js) — the old path
      // 404'd, so this list silently never loaded anything in production.
  ...
  const handleDownload = async (pdf) => {
    ...
    // Fire-and-forget view ping — counts a distinct "seen" per student
    API.post(`/pdfs/${pdf.id}/view`).catch(() => {});
    try {
      const token = localStorage.getItem("token");
      const apiBase = process.env.REACT_APP_API_URL || "";

      const res = await fetch(`${apiBase}/pdfs/${pdf.id}/download`, {
  ...
  const handleReact = async (pdfId, reaction) => {
    const res = await API.post(`/pdfs/${pdfId}/react`, { reaction });
    return res.data;
  };
  ...
  const s = {
    ...
    searchInput: {
      ...
      background: "var(--surface)", border: "1px solid #374151",
