...
export default function Profile() {
  ...
  useEffect(() => {
    if (student) {
      ...
      setUsernameInput(student.username || "");
    }
  ...
  const saveUsername = async () => {
    ...
    if (clean === (student?.username || "")) return; // nothing changed
    if (clean.length < 3 || clean.length > 20)
      return showMsg("error", "Username must be 3–20 characters long.");
    if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(clean))
      return showMsg("error", "Username must start with a letter and use only letters, numbers, and underscores.");

    setSavingUsername(true);
    try {
      const res = await API.put("/auth/username", { username: clean });
      if (res.data?.token) localStorage.setItem("token", res.data.token);
      await refreshStudent();
      showMsg("success", "Username updated!");
    } catch (err) {
      showMsg("error", err.response?.data?.error || "Failed to update username.");
      ...
      <div style={s.container}>
        ...
        <div style={s.profileHeader}>
          ...
          {showAvatars && (
            <div style={s.avatarPicker}>
              <p style={{ margin:"0 0 8px", fontSize:12, color:"var(--text-muted)", fontWeight:600 }}>Choose Avatar</p>
          ...
          <div>
            ...
            {student?.username && (
              <div style={{ color:"var(--primary)", fontSize:13, fontWeight:600, marginBottom:2 }}>@{student.username}</div>
            )}
            <div style={{ color:"var(--text-muted)", fontSize:14 }}>{student?.email}</div>
            <div style={{ color:"var(--text-muted)", fontSize:13, marginTop:2 }}>📱 {student?.phone}</div>
            {student?.is_premium && (
        ...
        <div style={s.tabs}>
          {[["profile","👤 Profile"],["security","🔒 Security"],["key","🔑 Activate Key"]].map(([id,label]) => (
            <button key={id}
              style={{ ...s.tab, background:tab===id?"#6c63ff":"#fff", color:tab===id?"#fff":"var(--text-muted)" }}
              onClick={() => setTab(id)}>
              {label}
            </button>
          ))}
        ...
        {tab === "profile" && (
            ...
            <div style={s.formGrid}>
              ...
              <div>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  ...
                  <button
                    style={{ ...s.saveBtn, width:"auto", padding:"0 16px", opacity:(savingUsername || usernameInput.trim()===(student?.username||""))?0.6:1 }}
                    disabled={savingUsername || usernameInput.trim()===(student?.username||"")}
                    onClick={saveUsername}>
                    {savingUsername ? "..." : "Save"}
                  </button>
                </div>
                <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:4 }}>
                  This is how other students find and add you. You can change it anytime.
                </div>
              </div>
              <div>
          ...
          {/* Parent Link Code */}
          <div style={{ marginTop: 16, background: "linear-gradient(135deg, #f0f4ff, #e8f0fe)", borderRadius: 12, padding: 16, border: "1px solid #6C63FF30" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ flex: 1, background: "var(--surface)", borderRadius: 8, padding: "10px 14px", fontWeight: 800, fontSize: 18, color: "#6C63FF", letterSpacing: 2, border: "1px solid #6C63FF30", textAlign: "center" }}>
                {student?.parent_link_code || "Loading..."}
              </div>
              <button
                onClick={() => { navigator.clipboard.writeText(student?.parent_link_code || ""); }}
                style={{ background: "#6C63FF", border: "none", color: "#fff", borderRadius: 8, padding: "10px 14px", cursor: "pointer", fontWeight: 700, fontSize: 13 }}>
                Copy
              </button>
            </div>
            <div style={{ color: "var(--text-muted)", fontSize: 11, marginTop: 8 }}>
              Parent portal: <strong>cbt-frontend-umber.vercel.app/parent</strong>
            </div>
          </div>
          </div>
        )}
        ...
        <div style={{ textAlign: "center", marginTop: 28, paddingTop: 18, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)", opacity: 0.7 }}>
            Scholars Syndicate · Founded &amp; led by Eli Tamuno-Owunari Tamunosiki, CEO
          </p>
        </div>
      </div>
    </div>
...
function KeyActivator({ onActivate }) {
  ...
  return (
    <div>
      <label style={{ display:"block", fontSize:13, fontWeight:600, color:"#f0f4ff", marginBottom:6 }}>
        Activation Key
      </label>
...
function StatPill({ icon, label, value }) {
  return (
    <div style={{ flex:1, background:"var(--surface)", borderRadius:10, padding:"12px 10px", textAlign:"center",
...
const s = {
  ...
  profileHeader: { display:"flex", alignItems:"flex-start", gap:20, background:"var(--surface)", borderRadius:14, padding:"20px", boxShadow:"0 2px 12px rgba(0,0,0,0.06)", marginBottom:14, flexWrap:"wrap", position:"relative" },
  ...
  editBadge:     { position:"absolute", bottom:0, right:0, background:"var(--surface)", border:"2px solid #dfe6e9", borderRadius:"50%", width:24, height:24, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12 },
  avatarPicker:  { position:"absolute", top:110, left:16, background:"var(--surface)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:14, boxShadow:"0 8px 24px rgba(0,0,0,0.12)", zIndex:100, width:280 },
  avatarBtn:     { fontSize:24, background:"var(--bg)", border:"none", borderRadius:8, padding:"6px 8px", cursor:"pointer", transition:"transform 0.2s", ":hover":{ transform:"scale(1.05)" } },
  ...
  card:          { background:"var(--surface)", borderRadius:14, padding:"20px 18px", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" },
  cardTitle:     { fontSize:16, fontWeight:800, color:"#f0f4ff", marginBottom:16 },
  formGrid:      { display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:8 },
  label:         { display:"block", fontSize:12, fontWeight:600, color:"var(--text-muted)", marginBottom:4, marginTop:10 },
  ...
  pricingBox:    { background:"var(--bg)", borderRadius:10, padding:"14px", marginTop:16 },
