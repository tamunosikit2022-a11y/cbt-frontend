...
function MissionCard({ mission, onClaim, claiming }) {
  ...
  const statusColor = mission.completed
    ? (mission.claimed ? "var(--text-muted)" : "#00b894")
    : "#6c63ff";
      ...
      {mission.target > 1 && (
        <div style={{ marginTop: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{mission.progress} / {mission.target}</span>
        ...
        <div style={s.actionGrid}>
          {[
            ...
            <div key={i} style={s.actionCard} onClick={() => nav(a.path)}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, marginTop: 6, color: "var(--text)" }}>{a.label}</span>
            </div>
          ))}
        </div>
...
const s = {
  ...
  summaryLbl:   { fontSize:11, color:"var(--text-muted)", marginTop:2 },

  // Tabs
  tabs:         { display:"flex", gap:8, marginBottom:14 },
  tab:          { flex:1, padding:"10px 0", background:"#fff", border:"2px solid #f0f0f0", borderRadius:12, fontWeight:700, fontSize:13, cursor:"pointer", color:"var(--text-muted)" },
  ...
  missionTitle: { fontWeight:800, fontSize:14, color:"var(--text)", marginBottom:3 },
  missionDesc:  { fontSize:12, color:"var(--text-muted)", lineHeight:1.4 },
