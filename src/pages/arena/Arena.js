...
const MODE_GROUPS = [
    ...
    modes: [
      {
        ...
        desc: "Casual 1v1 · No rank at stake",
        detail: "Quick casual match. No rank points lost or gained. Great for warming up.",
        badge: "CASUAL", badgeColor: "#00b894",
        maxP: 2, color: "#00b894",
      },
      {
        id: "duel", icon: "⚔️", name: "Duel",
        desc: "Ranked 1v1 · XP on the line",
        detail: "Ranked match. Win to earn XP and climb from Bronze to Elite. Your record counts.",
        badge: "RANKED", badgeColor: "#e17055",
        maxP: 2, color: "#e17055",
      },
      {
        id: "duo", icon: "👥", name: "Duo",
        desc: "2v2 team battle",
        detail: "Team up with a friend vs another pair. Combined score decides the winner.",
        badge: "TEAM", badgeColor: "#0984e3",
        maxP: 4, color: "#0984e3",
      },
    ],
  },
  {
    ...
    modes: [
      {
        id: "clash_squad", icon: "🛡️", name: "Clash Squad",
        desc: "2 teams · 4 each · 8 total",
        detail: "Two squads of 4 face off. Coordinate with your team for maximum score.",
        badge: "SQUAD", badgeColor: "#00b894",
        maxP: 8, color: "#00b894",
      },
    ],
  },
  {
    ...
    modes: [
      {
        id: "battle_royal", icon: "👑", name: "Battle Royal",
        desc: "Up to 50 players · free for all",
        detail: "Every player for themselves. Up to 50 students, highest score takes the crown.",
        badge: "FREE FOR ALL", badgeColor: "#fdcb6e",
        maxP: 50, color: "#fdcb6e",
      },
    ],
  },
];
      ...
      <div style={s.container}>
        ...
        {showCreateGate && (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}
            ...
            <div style={{ background:"var(--surface)", border:"1.5px solid #6c63ff55", borderRadius:22, padding:"32px 24px", maxWidth:360, width:"100%", textAlign:"center" }}
              ...
              <div style={{ background:"var(--surface)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:"12px 16px", marginBottom:18, display:"flex", justifyContent:"space-around" }}>
                {[{ label:"Trial", price:"₦100", sub:"3 hrs" }, { label:"Weekly", price:"₦700", sub:"7 days" }, { label:"Monthly", price:"₦2,000", sub:"30 days" }].map((p,i) => (
                  <div key={i} style={{ textAlign:"center" }}>
                    ...
                    <div style={{ fontSize:10, color:"var(--text-muted)" }}>{p.sub}</div>
                  </div>
                ))}
              </div>
              <button onClick={() => nav("/subscribe")} style={{ width:"100%", padding:"13px 0", background:"linear-gradient(135deg,#6c63ff,#a29bfe)", color:"#fff", border:"none", borderRadius:12, fontWeight:800, fontSize:14, cursor:"pointer", marginBottom:10, boxShadow:"0 6px 20px rgba(108,99,255,0.35)" }}>
                👑 Upgrade to Create Rooms →
              </button>
              <button onClick={() => setShowCreateGate(false)} style={{ width:"100%", padding:"11px 0", background:"none", border:"1.5px solid rgba(255,255,255,0.12)", borderRadius:12, color:"var(--text-muted)", fontWeight:600, fontSize:13, cursor:"pointer" }}>
                Continue Browsing
              </button>
            </div>
          </div>
              ...
              {!student?.is_premium && (
                <div style={{ background:"rgba(108,99,255,0.1)", border:"1px solid rgba(108,99,255,0.3)", borderRadius:10, padding:"10px 13px", marginBottom:10, display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:1 }}>You can join any open room. Hosting requires Premium from ₦100.</div>
                  </div>
                  <button onClick={() => nav("/subscribe")} style={{ background:"#6c63ff", border:"none", borderRadius:8, color:"#fff", fontWeight:800, fontSize:11, padding:"6px 10px", cursor:"pointer", flexShrink:0 }}>
                    Upgrade
                  </button>
                </div>
              )}
                ...
                <div key={group.label} style={{ marginBottom: 8 }}>
                  ...
                  <div
                    style={{
                      ...
                      background: group.modes.some(m => m.id === mode) ? group.color + "15" : "var(--surface)",
                    }}
                    onClick={() => {
                      ...
                      {group.modes.map(m => (
                        <div key={m.id}
                          style={{
                            ...
                            background: mode === m.id ? m.color + "22" : "var(--bg)",
                          }}
                          ...
                          <div style={{ flex:1 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:2 }}>
                              ...
                              <span style={{ fontSize:9, fontWeight:800, color: m.badgeColor, background: m.badgeColor+"22", padding:"1px 6px", borderRadius:6 }}>{m.badge}</span>
                            </div>
                            <div style={{ fontSize: 11, color: "#636e72" }}>{m.desc}</div>
                            <div style={{ fontSize: 10, color: "#4a4a6a", marginTop:2, lineHeight:1.5 }}>{m.detail}</div>
                          </div>
                          {mode === m.id && <span style={{ marginLeft: "auto", color: m.color, fontSize: 14, flexShrink:0 }}>✓</span>}
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {mode && (
                <div style={s.configPanel}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                    {BATTLE_TYPES.map(b => (
                      <div key={b.id}
                        style={{ ...s.btChip, borderColor: battleType === b.id ? "#e17055" : "#2d2d44", background: battleType === b.id ? "#e1705520" : "var(--bg)", flex: 1 }}
...
function MiniStat({ label, value, color="#a29bfe" }) {
  return (
    <div style={{ flex:1, background:"var(--surface)", borderRadius:10, padding:"8px 6px", textAlign:"center", border:"1px solid #2d2d44" }}>
...
const s = {
  page:         { minHeight:"100vh", background:"var(--bg)", fontFamily:"sans-serif", padding:16, animation:"fadeInUp 0.35s ease" },
  container:    { maxWidth:960, margin:"0 auto" },
  header:       { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 },
  back:         { background:"none", border:"none", color:"#a29bfe", fontWeight:700, cursor:"pointer", fontSize:14 },
  rankBadge:    { display:"flex", gap:6, alignItems:"center", background:"var(--surface)", padding:"5px 12px", borderRadius:20, border:"1px solid #2d2d44" },
  ...
  liveCard:     { background:"var(--surface)", border:"1px solid #2d2d44", borderRadius:10, padding:"10px 12px", minWidth:140, flexShrink:0 },
  ...
  // Play layout
  playLayout:   { display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:14, alignItems:"start" },
  ...
  shelfHeader:  { display:"flex", alignItems:"center", gap:10, background:"var(--surface)", border:"2px solid", borderRadius:10, padding:"11px 14px", cursor:"pointer", marginBottom:2 },
  subModes:     { background:"var(--bg)", borderRadius:"0 0 10px 10px", border:"1px solid #2d2d44", borderTop:"none", padding:"8px 8px", display:"flex", flexDirection:"column", gap:6, marginBottom:6 },
  subModeChip:  { display:"flex", alignItems:"center", gap:10, border:"2px solid", borderRadius:8, padding:"8px 10px", cursor:"pointer" },

  // Config panel
  configPanel:  { background:"var(--surface)", border:"1px solid #2d2d44", borderRadius:12, padding:"14px", marginTop:10 },
  ...
  // Selects
  select:       { width:"100%", padding:"8px 10px", background:"var(--bg)", border:"1px solid #2d2d44", borderRadius:8, fontSize:13, color:"#fff", boxSizing:"border-box" },
  ...
  // Panel / join
  panel:        { background:"var(--surface)", borderRadius:14, padding:"16px", border:"1px solid #2d2d44" },
  ...
  pubRoom:      { display:"flex", alignItems:"center", gap:10, background:"var(--bg)", borderRadius:10, padding:"10px 12px", border:"1px solid #2d2d44" },
