...
export default function SpinWheel() {
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", background:"#0B1020", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ color:"rgba(255,255,255,0.5)", fontSize:14, fontFamily:"sans-serif" }}>Loading spin wheel...</div>
    </div>
  );
      ...
      <div style={s.body}>
        ...
        {showResult && result && (
          <div style={s.resultCard}>
            ...
            <div style={{ fontSize:28, fontWeight:900, color: result.type==="gems" ? "#00D4FF" : result.type==="xp" ? "#7C5CFF" : result.type==="boost" ? "#00D084" : "#FFC857", marginBottom:14, textShadow:`0 0 20px currentColor` }}>
              {result.label}
            </div>
            <div style={{ fontSize:13, color:"rgba(255,255,255,0.6)", marginBottom:16 }}>Added to your account!</div>
            <div style={s.balRow}>
              <div style={s.bal}><span>🪙</span> {coins.toLocaleString()}</div>
              <div style={{ ...s.bal, background:"rgba(0,212,255,0.15)", border:"1px solid rgba(0,212,255,0.3)", color:"#00D4FF" }}>
                <span>💎</span> {gems}
...
const s = {
  ...
  spinBtn:       { width:"100%", maxWidth:320, padding:"16px 0", fontSize:18, fontWeight:900, color:"#0B1020", background:"linear-gradient(135deg,#FFC857,#FFB300)", border:"none", borderRadius:16, cursor:"pointer", boxShadow:"0 4px 24px rgba(255,200,87,0.5)", marginBottom:20, letterSpacing:1 },
  spinBtnDisabled:{ background:"rgba(255,255,255,0.1)", color:"rgba(255,255,255,0.4)", boxShadow:"none", cursor:"not-allowed" },
  ...
  rewardsCard:   { background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:"16px", width:"100%", maxWidth:320 },
  rewardsTitle:  { fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.5)", letterSpacing:1.2, textTransform:"uppercase", marginBottom:12 },
  rewardsGrid:   { display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 },
  rewardItem:    { textAlign:"center", background:"rgba(255,255,255,0.04)", borderRadius:10, padding:"8px 4px" },
};
