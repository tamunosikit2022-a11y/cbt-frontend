...
// Fetches the protected PDF endpoint with the parent's bearer token (a plain
// <a href> can't send auth headers) and triggers a normal browser download.
async function downloadReportPDF(token) {
  try {
    const res = await fetch(`${API}/parent/report.pdf`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    alert("Couldn't download the report. Please try again.");
  }
}
    ...
    <div style={{minHeight:"100dvh",background:C.bg,fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",color:C.text}}>
      ...
      <div style={{background:"var(--nav-bg)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",borderBottom:`1px solid rgba(79,126,247,0.1)`,padding:"12px 16px",position:"sticky",top:0,zIndex:30,display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"0 2px 16px rgba(0,0,0,0.35)"}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={()=>downloadReportPDF(token)} title="Download progress report" style={{background:"rgba(0,212,170,0.1)",border:"1px solid rgba(0,212,170,0.3)",color:C.cyan,borderRadius:10,padding:"7px 12px",cursor:"pointer",fontSize:12.5,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",gap:5}}>📄 PDF</button>
