...
function generateCertificate({ studentName, subject, score, total, percentage, examType, institution, date, jambScore }) {
  ...
  // "This certifies that"
  ctx.font = "16px Georgia"; ctx.fillStyle = "var(--text-muted)";
  ...
  if (jambScore) {
    ...
    subjects.forEach(([subj, data], i) => {
      ...
      ctx.font = "11px Arial"; ctx.fillStyle = "var(--text-muted)";
  ...
  ctx.font = "13px Arial"; ctx.fillStyle = "var(--text-muted)";
...
export default function Results() {
  ...
  if (!state?.result) {
    return (
      <div style={{
        ...
        background: "var(--bg)", color: "var(--text-muted)", fontFamily: "'Segoe UI',sans-serif",
        textAlign: "center", padding: 24,
      }}>
        <div style={{ fontSize: 40 }}>📄</div>
        <p>No result to show. Redirecting to your dashboard…</p>
      </div>
    );
  }

  const { result, questions, subject, subjectMap, isJAMBFull, isPostUTME, institution, exam_type } = state;
  ...
  const handleGenerateCert = () => {
    ...
    setTimeout(() => {
      ...
      setGenerating(false);
      if (!img) {
        alert("Sorry, certificates aren't supported on this device/browser.");
        return;
      }
      setCertImg(img);
    ...
    <div style={s.page}>
      <div style={s.container}>
        ...
        <div style={{ ...s.scoreCard, borderTop:`6px solid ${grade.color}` }}>
          <ScoreRing percent={parseFloat(displayPct) || 0} color={grade.color} />

          {isJAMBFull && jambScore ? (
            <>
              <div style={{ fontSize:20, fontWeight:700, color:"#f0f4ff", marginTop:10 }}>{grade.label}</div>
              <div style={{ fontSize:14, color:"var(--text-muted)", marginTop:2 }}>{jambScore.totalScore} out of {jambScore.maxScore}</div>
              <p style={{ color:"var(--text-muted)", marginTop:6, fontSize:14 }}>JAMB / UTME Aggregate Score</p>
            </>
          ) : (
            <>
              <div style={{ fontSize:20, fontWeight:700, color:"#f0f4ff", marginTop:10 }}>{grade.label}</div>
          ...
          <div style={s.statsRow}>
            ...
            <StatBit label="Unanswered" value={unansweredCount} color="var(--text-muted)" />
          </div>
        </div>
        ...
        <div style={s.certSection}>
          <div style={s.certHeader}>
            <div>
              <div style={{ fontWeight:800, fontSize:16, color:"#f0f4ff" }}>🏅 Score Certificate</div>
              <div style={{ fontSize:12, color:"var(--text-muted)", marginTop:3 }}>Download & share your achievement on WhatsApp</div>
            </div>
            {!certImg && (
            ...
            <div style={s.breakGrid}>
              {Object.entries(jambScore.subjectScores).map(([subj, data]) => (
                <div key={subj} style={s.breakCard}>
                  <div style={{ fontSize:12, color:"var(--text-muted)", marginBottom:4 }}>{subj}</div>
                  <div style={{ fontSize:28, fontWeight:800, color:scoreColor(data.score) }}>{data.score}</div>
                  <div style={{ fontSize:11, color:"var(--text-muted)" }}>/100 · {data.correct}/{data.total} correct</div>
        ...
        <div style={s.actions}>
          ...
          <button style={{ ...s.btn, background:"var(--text)" }} onClick={() => nav("/dashboard")}>🏠 Dashboard</button>
          <button style={{ ...s.btn, background:"#25D366" }} onClick={() => setShowShare(true)}>📤 Share</button>
          {exam_type === "UNIVERSITY" && institution && (
            <button
              style={{ ...s.btn, background:"#F59E0B" }}
              onClick={() => nav(`/university-leaderboard?institution=${encodeURIComponent(institution)}`)}
            >
              🏆 See {institution} Scoreboard
            </button>
          )}
        </div>
          ...
          return wrongCount > 0 ? (
            <div style={{ background:"linear-gradient(135deg,#6c63ff15,#e1700520)", border:"2px solid #6c63ff40", borderRadius:14, padding:"16px 16px", marginBottom:16, display:"flex", flexDirection:"column", gap:10 }}>
              <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                ...
                <div>
                  <div style={{ fontWeight:800, fontSize:14, color:"#f0f4ff", marginBottom:3 }}>
                    You got {wrongCount} wrong — track your weak spots
                  </div>
                  <div style={{ fontSize:12, color:"var(--text-muted)" }}>
                    Premium saves these to your personal error bank, unlocks your weakness heatmap, and tracks improvement over time.
                  </div>
                </div>
              </div>
              <button
                ...
                👑 Unlock Error Tracker & Heatmap — from ₦100
              </button>
            </div>
          ) : null;
        })()}
        ...
        <p style={{ color:"var(--text-muted)", fontSize:13, marginBottom:16 }}>
          ✅ Correct answers shown below · Explanations included for all questions.
        </p>
            ...
            return (
              <div key={a.question_id || i}
                ...
                <div style={s.answerHeader}>
                  <span style={{ fontWeight:700, fontSize:13, color:"var(--text-muted)" }}>Q{i+1}</span>
                  <span style={{ ...s.correctBadge, background:a.is_correct ? "#e8f8f5" : a.selected_answer ? "rgba(239,68,68,0.1)" : "#f0f0f0", color:a.is_correct ? "#00b894" : a.selected_answer ? "#e17055" : "var(--text-muted)" }}>
                ...
                <div style={s.optionsRow}>
                  {["A","B","C","D"].map(opt => {
                    ...
                    return (
                      <span key={opt} style={{ ...s.optChip, background:isCorrect ? "#00b894" : isSelected ? "#e17055" : "#f0f0f0", color:(isCorrect||isSelected) ? "#fff" : "var(--text-muted)", fontWeight:isCorrect ? 700 : 400 }}>
...
// Circular progress ring, matching the mockup's donut-style score display.
  ...
  return (
    <div style={{ position:"relative", width:size, height:size, margin:"0 auto" }}>
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
        <div style={{ fontSize:32, fontWeight:800, color, lineHeight:1 }}>{Math.round(clamped)}%</div>
...
const s = {
  page:         { minHeight:"100vh", background:"var(--bg)", fontFamily:"'Segoe UI',sans-serif", padding:16 },
  container:    { maxWidth:760, margin:"0 auto", padding:"0 4px" },
  scoreCard:    { background:"var(--surface)", borderRadius:16, padding:"32px 24px", textAlign:"center", border:"1px solid rgba(255,255,255,0.07)", marginBottom:16 },
  ...
  certSection:  { background:"var(--surface)", borderRadius:16, padding:"18px 16px", boxShadow:"0 2px 16px rgba(108,99,255,0.1)", marginBottom:16, border:"2px solid #6c63ff22" },
  ...
  downloadBtn:  { flex:1, padding:"11px 0", background:"var(--text)", color:"#fff", border:"none", borderRadius:10, fontWeight:700, fontSize:13, cursor:"pointer" },
  ...
  regenBtn:     { padding:"11px 14px", background:"var(--surface)", color:"var(--text-muted)", border:"none", borderRadius:10, fontWeight:700, fontSize:13, cursor:"pointer" },
  shareHint:    { fontSize:12, color:"var(--text-muted)", textAlign:"center", padding:"4px 0 8px" },

  breakdown:    { background:"var(--surface)", borderRadius:14, padding:20, boxShadow:"0 2px 12px rgba(0,0,0,0.06)", marginBottom:16 },
  ...
  breakCard:    { background:"var(--bg)", borderRadius:10, padding:"12px 14px" },
  breakBar:     { height:6, background:"#e0e0e0", borderRadius:3, marginTop:8, overflow:"hidden" },
  breakFill:    { height:"100%", borderRadius:3 },
  jambNote:     { background:"var(--bg)", borderRadius:10, padding:"12px 14px", fontSize:13 },
  ...
  answerRow:    { background:"var(--surface)", borderRadius:12, padding:"14px 16px", boxShadow:"0 1px 8px rgba(0,0,0,0.05)" },
  ...
  answerQ:      { fontSize:14, color:"#f0f4ff", marginBottom:10, lineHeight:1.5 },
  optionsRow:   { display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 },
  optChip:      { padding:"4px 10px", borderRadius:6, fontSize:12 },
  expl:         { background:"#e8f8f5", borderRadius:8, padding:"10px 12px", fontSize:13, color:"#0F3D34", lineHeight:1.6 },
};
