...
export default function Exam() {
  ...
  // FIX BUG 1: Submit lock to prevent double submission
  const submitLock = useRef(false);

  // Keep ref in sync — fixes stale closure bug on submit
      ...
      {multiSubjects && multiSubjects.length > 1 && (
        <div style={s.subjectTabs}>
          {multiSubjects.map(subj => {
            ...
            return (
              <button key={subj}
                style={{ ...s.subjectTab, background: currentSubj === subj ? "#6c63ff" : "#fff", color: currentSubj === subj ? "#fff" : "#636e72" }}
        ...
        <div style={s.palette}>
          ...
          {multiSubjects ? (
              ...
              <div key={subj} style={{ marginBottom: 10 }}>
                ...
                <div style={s.paletteGrid}>
                  {(subjectMap[subj] || []).map((q2, i) => {
                    ...
                    return (
                      <button key={q2.id}
                        style={{ ...s.paletteNum, background: flagged[q2.id] ? "#fdcb6e" : answers[q2.id] ? "#00b894" : gi === current ? "#6c63ff" : "#f0f0f0", color: (answers[q2.id] || gi === current || flagged[q2.id]) ? "#fff" : "#2d3436" }}
            ...
            <div style={s.paletteGrid}>
              {questions.map((q2, i) => (
                <button key={q2.id}
                  style={{ ...s.paletteNum, background: flagged[q2.id] ? "#fdcb6e" : answers[q2.id] ? "#00b894" : i === current ? "#6c63ff" : "#f0f0f0", color: (answers[q2.id] || i === current || flagged[q2.id]) ? "#fff" : "#2d3436" }}
            ...
            {["A","B","C","D"].map(opt => {
              ...
              return (
                <div key={opt}
                  ...
                  <span style={{ ...s.optLabel, background: selected ? "#6c63ff" : "#f0f0f0", color: selected ? "#fff" : "#2d3436" }}>
                    {opt}
                  </span>
...
const s = {
  ...
  topBar:       { background: "#fff", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", flexWrap: "wrap", gap: 8, position: "sticky", top: 0, zIndex: 100 },
  topLeft:      { display: "flex", gap: 8, alignItems: "center" },
  topRight:     { display: "flex", gap: 8, alignItems: "center" },
  examBadge:    { background: "#f0edff", color: "#6c63ff", padding: "3px 10px", borderRadius: 8, fontSize: 12, fontWeight: 700 },
  timer:        { border: "2px solid", borderRadius: 8, padding: "4px 12px", fontWeight: 800, fontSize: 16 },
  paletteBtn:   { background: "#f0f0f0", border: "none", borderRadius: 8, padding: "7px 12px", cursor: "pointer", fontSize: 13, fontWeight: 600 },
  ...
  progressBg:   { height: 4, background: "#dfe6e9" },
  progressFill: { height: "100%", background: "#6c63ff", transition: "width 0.3s" },
  subjectTabs:  { background: "#fff", display: "flex", gap: 4, padding: "8px 12px", overflowX: "auto", borderBottom: "1px solid #f0f0f0" },
  subjectTab:   { border: "2px solid #dfe6e9", borderRadius: 8, padding: "6px 12px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, whiteSpace: "nowrap", minWidth: 80 },
  palette:      { background: "#fff", padding: 16, boxShadow: "0 4px 16px rgba(0,0,0,0.1)", position: "sticky", top: 60, zIndex: 99, maxHeight: "40vh", overflowY: "auto" },
  ...
  qCard:        { background: "#fff", borderRadius: 14, padding: "24px 20px", boxShadow: "0 2px 16px rgba(0,0,0,0.07)", marginBottom: 16 },
  qHeader:      { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 },
  qNum:         { fontSize: 13, fontWeight: 700, color: "#6c63ff" },
  flagBtn:      { background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600 },
  metaTag:      { fontSize: 11, padding: "2px 8px", borderRadius: 6, background: "#f0f0f0", color: "#636e72" },
  qText:        { fontSize: 16, lineHeight: 1.7, color: "#2d3436", marginBottom: 20 },
  options:      { display: "flex", flexDirection: "column", gap: 10 },
  option:       { display: "flex", alignItems: "center", gap: 12, padding: "14px 14px", border: "2px solid #dfe6e9", borderRadius: 10, cursor: "pointer", minHeight: 52 },
  optSelected:  { border: "2px solid #6c63ff", background: "#f0edff" },
  ...
  explanation:  { marginTop: 16, background: "#e8f8f5", borderRadius: 8, padding: "12px 14px", fontSize: 14, color: "#2d3436", lineHeight: 1.6 },
  navRow:       { display: "flex", justifyContent: "space-between", alignItems: "center" },
  navBtn:       { padding: "11px 22px", border: "2px solid #dfe6e9", borderRadius: 10, fontWeight: 700, cursor: "pointer", background: "#fff", fontSize: 14 },
};