      ...
      <div style={s.container}>
        ...
        <div style={s.tabs}>
          {["all","JAMB","POST-UTME"].map(f => (
            <button key={f}
              style={{ ...s.tab, background: filter === f ? "#6c63ff" : "var(--surface)", color: filter === f ? "#fff" : "var(--text-muted)", border: filter === f ? "none" : "1px solid rgba(255,255,255,0.12)" }}
              onClick={() => setFilter(f)}>
              {f === "all" ? "All" : f}
            </button>
          ))}
        ...
        {filtered.length > 0 && (
          <div
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12, color:"var(--text-muted)" }}>Use AI Tutor to understand why you got questions wrong. 1 token per message.</div>
            </div>
...
function SumCard({ label, value, color, icon }) {
  return (
    <div style={{ flex: 1, background: "var(--surface)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "14px 16px", textAlign: "center" }}>
...
const s = {
  ...
  tab:       { padding: "8px 16px", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, fontWeight: 600, cursor: "pointer", fontSize: 13, background: "var(--surface)", color: "var(--text-muted)" },
  list:      { display: "flex", flexDirection: "column", gap: 10 },
  row:       { background: "var(--surface)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 },
