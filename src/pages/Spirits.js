...
const RARITY_COLORS = {
  ...
  epic:      { bg:"rgba(124,92,255,.1)",    border:"rgba(124,92,255,.5)",    text:"#7C5CFF",  glow:"#7C5CFF" },
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#0B1020" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ color:"#7C5CFF", marginTop:16, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Summoning Spirits…</p>
      </div>
    </div>
  );
      ...
      {modal && selected && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:1000,
          ...
          <div style={{ background:"#151B2E", borderRadius:24, padding:28, maxWidth:380, width:"100%",
            ...
            boxShadow:`0 0 60px ${RARITY_COLORS[selected.rarity]?.glow || "#7C5CFF"}30`,
            ...
            <div style={{ background:"rgba(124,92,255,.06)", border:"1px solid rgba(124,92,255,.2)",
              borderRadius:12, padding:"12px 16px", marginBottom:10 }}>
              <p style={{ color:"#7C5CFF", fontWeight:700, fontSize:11, letterSpacing:1, margin:0 }}>
              ...
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {selected.evolutions?.map((ev, i) => (
                  <div key={i} style={{ padding:"4px 10px", borderRadius:20, fontSize:11, fontWeight:600,
                    ...
                    color: i <= (selected.evolution_stage || 0) ? "#7C5CFF" : "rgba(255,255,255,.3)",
          ...
          {rarities.map(r => (
            <button key={r} onClick={() => setFilter(r)}
              style={{ padding:"6px 16px", borderRadius:20, border:"none", cursor:"pointer", fontWeight:700,
                ...
                background: filter===r ? "#7C5CFF" : "rgba(255,255,255,.06)",
