...
import AdminCutoffPanel from "../../components/AdminCutoffPanel";
...
const TABS = [
  ...
  { id: "cutoffs",       icon: "🎯", label: "Cutoff Marks" },
  { id: "duplicates",    icon: "🧬", label: "Duplicate Accounts" },
];
  ...
  return (
    <div style={st.page}>
      ...
      <div style={st.main}>
        ...
        {tab === "duplicates" && (
          <DuplicateAccountsPanel API_URL={API_URL} token={localStorage.getItem("admin_token")} />
        )}
...
// ── DUPLICATE ACCOUNTS PANEL ────────────────────────────────
function DuplicateAccountsPanel({ API_URL, token }) {
  ...
  const load = () => {
    setLoading(true);
    fetch(`${API_URL}/admin/duplicate-accounts`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => { setGroups(d.groups || []); setError(""); })
      .catch(() => setError("Failed to load duplicate accounts."))
      .finally(() => setLoading(false));
  };

  useEffect(() => { load(); }, []); // eslint-disable-line

  const resolve = async (group, keepId) => {
    const dupIds = group.accounts.map(a => a.id);
    if (!window.confirm(`Ban ${dupIds.length - 1} account(s) in this group and keep #${keepId}? This cannot be easily undone.`)) return;
    setBusyKey(group.match_key);
    try {
      const res = await fetch(`${API_URL}/admin/duplicate-accounts/merge-ban`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ keep_id: keepId, duplicate_ids: dupIds }),
      });
      if (!res.ok) throw new Error();
      load();
    } catch {
      alert("Failed to resolve group. Try again.");
    } finally {
      setBusyKey(null);
    }
  };

  const matchLabel = { phone: "Same Phone", email: "Same Email", name_school: "Same Name + School" };

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div>
          <h3 style={{ margin: 0 }}>Duplicate Accounts</h3>
          <div style={{ fontSize: 12, color: "#8b9cbd", marginTop: 2 }}>
            Groups sharing the same phone, email, or name+school. Ban all but the account you keep.
          </div>
        </div>
        <button onClick={load} style={{ background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 8, padding: "8px 14px", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 12.5 }}>
          ↻ Refresh
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ color: "#8b9cbd", padding: 20, textAlign: "center" }}>No duplicate accounts detected. 🎉</div>
      ) : (
        groups.map((g) => (
          <div key={g.match_key} style={{ background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: 16, marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 10, background: "#7C5CFF22", color: "#A98BFF" }}>
                  {matchLabel[g.match_type] || g.match_type}
                </span>
                <span style={{ marginLeft: 8, fontSize: 12.5, color: "#8b9cbd" }}>{g.match_key}</span>
              </div>
              <span style={{ fontSize: 12, color: "#8b9cbd" }}>{g.count} accounts</span>
            </div>

            <div style={{ display: "grid", gap: 8 }}>
              {g.accounts.map(a => (
                <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 12px" }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 13 }}>
                      #{a.id} {a.full_name} {a.is_banned && <span style={{ color: "#EF4444", fontSize: 11 }}>(banned)</span>}
                    </div>
                    <div style={{ fontSize: 11, color: "#8b9cbd" }}>
                      {a.email || "no email"} · {a.phone || "no phone"} · {a.exams_taken} exams · {a.token_balance ?? 0} tokens · joined {new Date(a.created_at).toLocaleDateString()}
                    </div>
                  </div>
                  {!a.is_banned && (
                    <button
                      onClick={() => resolve(g, a.id)}
                      disabled={busyKey === g.match_key}
                      style={{ background: "#00D4AA22", color: "#00D4AA", border: "none", borderRadius: 8, padding: "6px 12px", fontWeight: 700, fontSize: 11.5, cursor: "pointer", whiteSpace: "nowrap" }}
                    >
                      Keep this one
                    </button>
                  )}
