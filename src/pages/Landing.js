...
function ParticlesCanvas() {
  ...
  useEffect(() => {
    ...
    const COLORS = ["#7C5CFF","#5B8CFF","#00D4FF","#00D084","#a29bfe","#ffffff"];
...
export default function Landing() {
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif", overflowX:"hidden", background:"#0B1020" }}>
      ...
      {/* ── FEATURES ────────────────────────────────────────────── */}
      <section style={{ padding:"60px 20px", background:"#0B1020" }}>
      ...
      <section style={{ background:"linear-gradient(135deg,#7C5CFF,#5B8CFF)", padding:"64px 20px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <button className="land-btn" onClick={() => nav("/register")} style={{ padding:"16px 40px", background:"#fff", color:"#7C5CFF", border:"none", borderRadius:14, fontWeight:900, fontSize:17, boxShadow:"0 8px 32px rgba(0,0,0,.25)", position:"relative" }}>
          Create Free Account →
        </button>
      </section>
