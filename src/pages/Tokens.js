...
export default function Tokens() {
  ...
  // Poll balance every 15s when there's a pending WhatsApp payment
  useEffect(() => {
    const hasPending = history.some(t => t.status === 'whatsapp_pending');
    if (!hasPending) return;
    const iv = setInterval(async () => {
      try {
        const b = await API.get('/tokens/balance');
        const newBal = b.data.token_balance;
        if (newBal !== balance) {
          setBalance(newBal);
          // Re-fetch history to update status
  ...
  return (
    <div style={{ minHeight:'100vh', background:BG, fontFamily:"'Plus Jakarta Sans',sans-serif", color:'#fff', paddingBottom:40 }}>
      ...
      {/* Tabs */}
      <div style={{ display:'flex', margin:'16px 16px 0', background:"var(--surface)", borderRadius:12, padding:4 }}>
