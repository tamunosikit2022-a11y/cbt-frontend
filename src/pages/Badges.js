          ...
          <div style={s.grid}>
            {data?.earned?.length === 0 && (
              <div style={s.empty}>
                ...
                <p style={{ color: "var(--text-muted)" }}>No badges yet. Take exams to start earning!</p>
...
function Loader() {
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      <p style={{ color: "var(--text-muted)" }}>⏳ Loading badges...</p>
    </div>
  );
}
...
const s = {
  ...
  title:       { fontSize: 24, fontWeight: 800, color: "#f0f4ff", marginBottom: 4 },
  sub:         { color: "var(--text-muted)", fontSize: 14, marginBottom: 12 },
  ...
  badgeCard:   { background: "var(--surface)", borderRadius: 14, padding: "18px 14px", textAlign: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 },
  badgeIcon:   { fontSize: 40 },
  badgeName:   { fontWeight: 800, fontSize: 13, color: "#f0f4ff" },
  badgeDesc:   { fontSize: 11, color: "var(--text-muted)", lineHeight: 1.4 },
  earnedDate:  { fontSize: 10, color: "#a29bfe", fontWeight: 600 },
  empty:       { gridColumn: "1/-1", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: 32, background: "var(--surface)", borderRadius: 14, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" },
