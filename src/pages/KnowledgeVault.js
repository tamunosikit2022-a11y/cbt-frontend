...
const RARITY_STYLE = {
  ...
  epic:      { bg:"rgba(124,92,255,.1)",  border:"rgba(124,92,255,.4)", text:"var(--primary)" },
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"var(--bg)" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ color:"var(--primary)", marginTop:16, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Opening Vault…</p>
      </div>
    </div>
  );
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.92)", zIndex:2000,
          ...
          <div style={{ background:"linear-gradient(135deg,#0f1729 0%,#1a1040 100%)", borderRadius:24,
            ...
            <p style={{ color:"rgba(255,255,255,.5)", fontSize:13, marginBottom:24, lineHeight:1.6 }}>
              You don't have enough coins or tokens to unlock <strong style={{color:"#FFC857"}}>{modal?.title}</strong>.
            ...
            <div style={{ background:"rgba(124,92,255,.1)", borderRadius:14, padding:16, marginBottom:20, textAlign:"left", border:"1px solid rgba(124,92,255,.2)" }}>
              <div style={{ color:"var(--primary)", fontWeight:800, fontSize:12, marginBottom:10, textTransform:"uppercase", letterSpacing:1 }}>
                Premium includes
              </div>
              {[
              ...
              return (
                <div key={item.id} style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(0,208,132,.2)",
                  ...
                  <button className="cyber-btn" style={{ padding:"10px 18px", fontSize:13,
                    ...
                    onClick={() => item.file_url ? window.open(item.file_url, "_blank") : showToast("This item doesn't have its file attached yet — check the Downloads tab for available PDFs in the meantime.", "error")}>
                    📖 Read
                  </button>
                </div>
              );
