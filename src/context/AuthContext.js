...
export function AuthProvider({ children }) {
  ...
  // IMPORTANT: setLoading(false) is called only AFTER the profile fetch resolves
  // so the app never renders with a stale is_premium=false from localStorage.
  // This eliminates the race condition where premium gates flash on page load
  // even during a free day event.
  useEffect(() => {
    const token = localStorage.getItem("token");
    const saved  = localStorage.getItem("student");

    if (token && saved) {
      // Pre-populate from cache so UI has something to render immediately
      try { setStudent(JSON.parse(saved)); }
      catch { localStorage.removeItem("student"); }

      // Always re-validate with server — this gives us the correct is_premium
      // (including free day events) before we release the loading gate.
      API.get("/auth/profile")
        ...
        .finally(() => setLoading(false));  // ← release loading gate AFTER fetch
    } else {
      setLoading(false);  // no token → show login immediately
    }
  }, []);
