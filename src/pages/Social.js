...
export default function Social() {
  ...
  const sendRequest = async (toId) => {
    try {
      ...
      flash(r.data.success !== false ? "✅ Friend request sent!" : "❌ " + r.data.error);
    } catch (err) {
      flash("❌ " + (err.response?.data?.error || "Couldn't send request."));
    }
  };
  ...
  const createSquad = async () => {
    ...
    try {
      ...
      if (r.data.success) { flash("✅ Squad created!"); load(); } else flash("❌ " + r.data.error);
    } catch (err) {
      flash("❌ " + (err.response?.data?.error || "Couldn't create squad."));
    }
  };
  ...
  const inviteToSquad = async (targetId) => {
    ...
    try {
      // FIX: this was a plain single-quoted string, so `${squad.id}` was
      // sent literally instead of being interpolated — every invite request
      // hit the non-existent URL "/social/squads/${squad.id}/invite" and
      // 404'd. Needed a template literal (backticks). Also fixed the
      // response handling below (axios responses use `.data`, not `.json()`).
      const r = await API.post(`/social/squads/${squad.id}/invite`, { targetId });
      flash(r.data.success ? "✅ Invite sent!" : "❌ " + r.data.error);
    } catch (err) {
      flash("❌ " + (err.response?.data?.error || "Couldn't send invite."));
    }
  };
  ...
  const acceptSquadInvite = async (inviteId) => {
    try {
      ...
      if (r.data.success) { flash("✅ Joined squad!"); load(); } else flash("❌ " + r.data.error);
    } catch (err) {
      flash("❌ " + (err.response?.data?.error || "Couldn't join squad."));
    }
  };
        ...
        <>
          ...
          {squad ? (
            <>
              ...
              {/* FIX: SquadChat was a fully working, complete component that
                  was never imported or rendered anywhere in the app — so
                  squad chat looked "broken" when really it just wasn't
                  wired up. It now shows here once a student is in a squad. */}
              <div style={{ marginTop: 16 }}>
                <SquadChat squadId={squad.id} />
              </div>
            </>
          ) : (
            <div style={st.card}>
