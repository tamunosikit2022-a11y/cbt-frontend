...
export default function AIQuestions() {
  ...
  const revealOne = (qi) => setRevealed(prev => ({ ...prev, [qi]: true }));

  const submitAll = () => setSubmitted(true);

  const score = Object.keys(answerKey).filter(i => answers[i] === answerKey[i]).length;
    ...
    <div style={s.page}>
      ...
      <div style={{ maxWidth:700, margin:"0 auto", padding:"16px 12px 80px" }}>
        ...
        {questions.map((q, qi) => {
          ...
          const correct   = answerKey[qi];
          const isCorrect = submitted && chosen === correct;
          const isWrong   = submitted && chosen && chosen !== correct;
          const show      = submitted || revealed[qi];
        ...
        {questions.length > 0 && !submitted && (
          <div style={{ display:"flex", justifyContent:"center", marginTop:8 }}>
            <button onClick={submitAll} style={{ ...s.btnPrimary, padding:"14px 40px" }}>
