...
export default function ExamSelect() {
  ...
  // ── DUAL EXAM MODE (fixed pattern vs custom) ──────────────
  // Only applies to single-subject JAMB practice — full JAMB sim and
  // Post-UTME stay fixed-pattern since those mirror real exam formats.
  const [patternMode,    setPatternMode]    = useState("fixed");       // "fixed" | "custom"
  const [customCount,    setCustomCount]    = useState(20);
  const [customTime,     setCustomTime]     = useState(20);
  const [customDifficulty, setCustomDifficulty] = useState("any");     // any | easy | medium | hard
          ...
          <>
            ...
            {jambMode === "full" ? (
              <>
                ...
                {/* Subject Selection */}
                <div style={s.group}>
                  {/* FIX (students never noticed this): the course guide
                      used to be a small text-style toggle button tucked
                      next to the label — easy to scroll straight past,
                      collapsed by default, and gave no indication it
                      existed until you happened to spot it. It's now an
                      always-visible banner shown before subject picking
                      even starts, with a direct link into the Career Quiz
                      for students who genuinely don't know what course
                      they want yet. */}
                  <div style={s.guideBanner}>
                    <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                      <span style={{ fontSize:22, lineHeight:1 }}>💡</span>
                      <div style={{ flex:1 }}>
                        <div style={{ fontWeight:800, fontSize:14, marginBottom:2 }}>Not sure which subjects to pick?</div>
                        <div style={{ fontSize:12.5, color:"var(--text-muted)", lineHeight:1.5 }}>
                          Tell us your intended course and we'll auto-fill the right combination — or take our 2-minute quiz if you're still deciding on a course.
                        </div>
                      </div>
                    </div>
                    <div style={{ display:"flex", gap:8, marginTop:12 }}>
                      <button style={s.guideBtn} onClick={() => setShowGuide(!showGuide)}>
                        {showGuide ? "Hide course list" : "📋 Show course guide"}
                      </button>
                      <button style={s.careerQuizBtn} onClick={() => nav("/career-quiz")}>
                        🧭 Take Career Quiz
                      </button>
                    </div>

                    {/* Course guide list — now shown inline in the banner */}
                    {showGuide && (
                      <div style={{ marginTop:12 }}>
                        {JAMB_COURSE_GUIDES.map((g, i) => (
                          <div key={i} style={s.guideItem} onClick={() => applyGuide(g)}>
                            ...
                            <div style={{ fontSize:11, color:"var(--text-muted)" }}>{g.subjects.join(" · ")}</div>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <label style={{ ...s.label, display:"block", marginBottom:8 }}>Choose Your 3 Subjects</label>
              ...
              <>
                <div style={s.group}>
                  ...
                  <p style={{ fontSize:12, color:"var(--text-muted)", marginTop:6 }}>40 questions · focused practice on one subject</p>
                </div>
...
function InfoRow({ k, v }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, padding:"4px 0", borderBottom:"1px solid #e8e4ff" }}>
      ...
      <span style={{ fontWeight:700, color:"#f0f4ff" }}>{v}</span>
    </div>
  );
}
...
function ModeSelector({ modes, mode, setMode }) {
  return (
    <div style={{ marginBottom:22 }}>
      <label style={{ display:"block", fontWeight:700, fontSize:14, marginBottom:8, color:"#f0f4ff" }}>Study Mode</label>
...
// ── STYLES ────────────────────────────────────────────────
const s = {
  page:           { minHeight:"100vh", background:"var(--bg)", fontFamily:"sans-serif", padding:"16px 16px 40px" },
  ...
  title:          { fontSize:26, fontWeight:900, marginBottom:4, color:"#f0f4ff" },
  sub:            { fontSize:13, color:"var(--text-muted)", marginBottom:24 },
  group:          { marginBottom:24 },
  label:          { display:"block", fontWeight:700, fontSize:14, marginBottom:8, color:"#f0f4ff" },
  tabs:           { display:"flex", gap:8, flexWrap:"wrap" },
  tab:            { padding:"10px 20px", border:"1px solid rgba(255,255,255,0.08)", borderRadius:8, fontWeight:600, cursor:"pointer", fontSize:14, background:"var(--surface)", color:"var(--text-muted)" },
  tabActive:      { background:"#6c63ff", color:"#fff", borderColor:"#6c63ff" },
  infoBox:        { background:"rgba(108,99,255,0.15)", border:"2px solid #6c63ff", borderRadius:12, padding:"14px 16px", marginBottom:20 },
  infoTitle:      { fontWeight:800, fontSize:14, color:"#6c63ff", marginBottom:10 },
  infoGrid:       { display:"flex", flexDirection:"column", gap:0 },
  fixedSubject:   { background:"#dfe6e9", borderRadius:8, padding:"10px 14px", fontSize:13, fontWeight:600, color:"var(--text-muted)", marginBottom:12 },
  guideBanner:    { background:"linear-gradient(135deg, rgba(108,99,255,0.14), rgba(108,99,255,0.05))", border:"2px solid #6c63ff", borderRadius:14, padding:14, marginBottom:16 },
  guideBtn:       { flex:1, background:"#6c63ff", border:"none", borderRadius:10, color:"#fff", fontWeight:700, fontSize:12.5, padding:"10px 12px", cursor:"pointer" },
  careerQuizBtn:  { flex:1, background:"var(--surface)", border:"2px solid #6c63ff", borderRadius:10, color:"#6c63ff", fontWeight:700, fontSize:12.5, padding:"10px 12px", cursor:"pointer" },
  guideBox:       { background:"var(--surface)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:12, marginBottom:12, maxHeight:220, overflowY:"auto" },
  guideItem:      { background:"var(--surface)", padding:"10px 12px", borderRadius:8, cursor:"pointer", marginBottom:6, border:"1px solid rgba(255,255,255,0.08)", transition:"background 0.15s" },
  subjectGrid:    { display:"flex", flexWrap:"wrap", gap:8, marginBottom:10 },
  subjectChip:    { padding:"8px 14px", borderRadius:20, border:"1px solid rgba(255,255,255,0.08)", fontSize:13, cursor:"pointer", background:"var(--surface)", fontWeight:500, transition:"all 0.15s" },
  ...
  selectionSummary: { background:"var(--bg)", borderRadius:8, padding:"8px 12px", fontSize:13, marginTop:4 },
  select:         { width:"100%", padding:12, border:"1px solid rgba(255,255,255,0.08)", borderRadius:8, fontSize:15, background:"var(--surface)" },
  uniGrid:        { display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(100px,1fr))", gap:8 },
  uniCard:        { padding:"10px 8px", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, cursor:"pointer", textAlign:"center", background:"var(--surface)" },
